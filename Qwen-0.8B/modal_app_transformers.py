...
class QwenServer:
    ...
    def move_to_gpu(self):
        ...
        self._gpu_warmup_ms = int((time.time() - t0) * 1000)
        print(f"[COLD_START] move_to_gpu elapsed_ms={self._gpu_warmup_ms}")
        try:
            import weave
            self._weave_client = weave.init("Aitf-dfk-3/log-qwen")
            self._weave = weave
    ...
    def generate(
        ...
        # Start Weave trace before inference to capture real latency
        weave_call = None
        try:
