...
APP_NAME = "ministral-8b-ws3"
MODEL_ID = "aitf-its-tim3-dfk/ministral-8b-ws3"
...
hf_cache = modal.Volume.from_name("ministral-8b-ws3-cache", create_if_missing=True)
...
class MinistralServer:
    ...
    def move_to_gpu(self):
        ...
        if os.environ.get("WANDB_API_KEY"):
            try:
                import weave
                weave.init("Aitf-dfk-3/ministral")
                self._weave = weave
    ...
    def generate(
        ...
        try:
            if self._weave:
                ...
                @_weave.op(name=f"ministral-8b-ws3-generate | {ts}")
...
def infer(payload: dict[str, Any]) -> dict[str, Any]:
    return MinistralServer().generate.remote(
