...
hf_cache = modal.Volume.from_name("ministral-8b-merged-ws3-cache", create_if_missing=True)
...
TEXT_CLASSIFICATION_INSTRUCTION_RAW = (
...
def build_text_classification_messages(
    klaim: str = "",
    fakta: str = "",
    ...
    user_parts = []
    if klaim.strip():
        user_parts.append(klaim.strip())
    if fakta.strip():
        user_parts.append(f"Artikel Rujukan: {fakta.strip()}")

    user_text = "\n\n".join(user_parts)
    return [
        {
            "role": "system",
            "content": [{"type": "text", "text": instruction or TEXT_CLASSIFICATION_INSTRUCTION_RAW}],
        },
        {
            "role": "user",
...
class MinistralMergedServer:
    ...
    def move_to_gpu(self):
        ...
        if os.environ.get("WANDB_API_KEY"):
            try:
                import weave
                self._weave_client = weave.init("Aitf-dfk-3/ministral-merged")
                self._weave = weave
    ...
    def generate(
        ...
        if text_classification:
            messages = build_text_classification_messages(
                klaim=klaim,
                fakta=fakta,
                instruction=text_classification_prompt,
            )
        elif messages:
...
def infer(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    text_classification = bool(
        payload.get("text_classification", False)
        or payload.get("dfk_text", False)
    ...
    text_classification_prompt = (
        payload.get("text_classification_prompt")
        or payload.get("text_classification_system_prompt")
        or payload.get("text_classification_instruction")
        or payload.get("dfk_text_prompt")
        or payload.get("dfk_text_instruction")
        or (system_prompt if text_classification else None)
    )
